...
// T66 — il detail è la quarta sostitutiva. Cornice di 14 righe: sei in più del
// reader — il marginTop dell'area di compilazione più le sue cinque righe
// (azione, prompt, modello, titolo, priorità). Sono righe FISSE aggiunte dentro
...
test('il frame del detail non supera mai le righe del terminale', () => {
  for (const rows of [20, 24, 30, 40, 60]) {
    assert.ok(detailCapacity(rows) + 14 + SLACK <= rows, `sforo a ${rows} righe`);
  }
});

test("detail: l'area di compilazione costa sei righe in più del reader", () => {
  ...
  for (const rows of [24, 40, 60]) {
    assert.equal(detailCapacity(rows), readerCapacity(rows) - 6, `a ${rows} righe`);
  }
});

// T91 — il campo di ricerca è l'unica riga CONDIZIONALE dentro l'overlay: le
// cinque dell'area di compilazione sono fisse e stanno in DETAIL_CHROME.
...
test('detail con ricerca aperta: il frame non supera le righe del terminale', () => {
  for (const rows of [20, 24, 30, 40, 60]) {
    assert.ok(detailCapacity(rows, true) + 15 + SLACK <= rows, `sforo a ${rows} righe`);
